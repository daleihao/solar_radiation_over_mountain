...
if nnz(t)
    ...
    horzDis = nan(size(lat));
    horzAzm = nan(size(lat));
end
if nargout>3
    varargout{1} = horzPts;
end
...
% check that distances are monotonic
assert(isequal(D,sort(D)) || isequal(D,sort(D,'descend')),...
    'check lats and lons, distances along profile must be monotonic')
...
if verLessThan('matlab','9.9')
    azm = atan2d(nanmedian(sind(az)),nanmedian(cosd(az))); %#ok<*NANMEDIAN>
else
    azm = atan2d(median(sind(az),'omitnan'),median(cosd(az),'omitnan'));
end
% MATLAB atan2d returns in range -180 to +180 whereas MATLAB distance
% returns in range 0 to 360
if azm<0
    azm = azm+360;
end
...
if nnz(tdz)==0
    ...
    if ~isscalar(azm)
        if verLessThan('matlab','9.9')
            ...
            azm = atan2d(median(sind(azm),'omitnan'),median(cosd(azm),'omitnan'));
        end
    end
    if azm<0
        azm = 360+azm;
